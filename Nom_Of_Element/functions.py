...
def group_devices(dev_list):
    ...
    while i < len(dev_list) - 1:
        ...
        if d1_dict == d2_dict:
            dev_list[i]['cnt'] += 1
            if dev_list[i+1]['list_position'] and dev_list[i+1]['list_position'] != '1':
...
def get_part_from_database(component_name):
    ...
    if not rs2.EOF:
        ...
        if rs2.Fields('AttributeValue').Value:
            return False, ""
    rs1.MoveFirst()
    result = True, rs1.Fields('AttributeValue').Value
    db_connection.Close()
    return result
    ...
    for att_id in att_ids:
        ...
        if att.GetName() == 'Дополнительная часть':
            ...
            if len(cmp_prmtrs) > 2:
                ...
                cmp_note = ''
            part_exist, name = get_part_from_database(cmp_name)
            if part_exist:
                device = {
                    'ref': '',
                    'name': name,
    ...
    if dev.IsTerminalBlock() or dev.IsAssembly() and dev.GetComponentName() == '':
        ...
        for id in dev_ids:
            ...
            if dev.GetAttributeValue("DO_NOT_ADD_TO_LIST") != '1':
                ...
                if dev_name != root_name:
                    dev_name = dev_name[dev_name.find('.') + 1:]
                    if list_position == '':
                        list_position = '1'
                else:
                    dev_name = ''
                device = {
                    'ref': dev_name, #[dev_name.find('.') + 1:] if dev_name != root_name else '',
